...
import com.mymusic.orvai.travel_with.WebRTC.util.AsyncHttpURLConnection;
...
/**
 * Negotiates signaling for chatting with https://appr.tc "rooms". Uses the client<->server specifics of the apprtc AppEngine webapp.
 * https://appr.tc 의 "방 번호"를 사용하여 시그널링 서버를 만듭니다. apprtc AppEngine을 통하여 클라이언트 <-> 서버 특성을 사용합니다.
 * <p>
 * To use: create an instance of this object (registering a message handler) and call connectToRoom().  Once room connection is established onConnectedToRoom() callback with room parameters is invoked.
 * Messages to other party (with local Ice candidates and answer SDP) can
 * be sent after WebSocket connection is established.
 * 사용방법: 이 객체의 인스턴스를 만들고, connectToRoom() 메서드를 호출합니다. 해당 방번호에 연결이 설정되면, room 매개 변수가있는 onConnectedToRoom() 콜백이 호출됩니다.
 */
