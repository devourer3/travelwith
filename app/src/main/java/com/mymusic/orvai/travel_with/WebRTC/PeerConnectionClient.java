...
package com.mymusic.orvai.travel_with.WebRTC;
 ...
 * 실질적으로 AppRTC에 ICECandidate를 생성하기 위한 클래스
 * <p>
 * <p>All public methods are routed to local looper thread. All PeerConnectionEvents callbacks are invoked from the same looper thread. This class is a singleton.
 * 모든 공용 메소드는 로컬 루퍼 스레드로 라우트됩니다. 모든 PeerConnectionEvents 인터페이스의 콜백은 동일한 루퍼 스레드에서 호출됩니다. 이 클래스는 싱글톤입니다.
 */
public class PeerConnectionClient {
    ...
    // Executor thread is started once in private factory and is used for all peer connection API calls to ensure new peer connection factory is created on the same thread as previously destroyed factory.
    // Executor 스레드는 PeerConnectionFactory에서 한번 시작되어, 모든 연결 API 호출에 사용됩니다. 새로운 PeerConnectionFactory가 이전에 파기된 팩토리와 동일한 스레드에 생성되도록 합니다.
    ...
    // Queued remote ICE candidates are consumed only after both local and remote descriptions are set. Similarly local ICE candidates are sent to remote peer after both local and remote description are set.
    // 대기중인 원격 ICE 후보(Candidates)는 로컬 및 원격 설명이 모두 설정된 후에 만 사용됩니다. 로컬 ICE 후보자(Candidates)는 로컬 및 원격 설명이 모두 설정된 후에 원격 피어로 전송됩니다.
    @Nullable
    ...
    public static class PeerConnectionParameters {
        ...
        public PeerConnectionParameters(boolean videoCallEnabled, boolean loopback, boolean tracing,
                                        ...
                                        String audioCodec, boolean noAudioProcessing, boolean aecDump, boolean useOpenSLES, boolean disableBuiltInAEC, boolean disableBuiltInAGC,
                                        boolean disableBuiltInNS, boolean disableWebRtcAGCAndHPF, boolean useLegacyAudioDevice, DataChannelParameters dataChannelParameters) {
...
//                            .setEnableVideoHwAcceleration(peerConnectionParameters.videoCodecHwAcceleration)                   // 수정했음!!!!!!!!!!!!!!
...
//        if (isVideoCallEnabled()) { // Fix됨
//            factory.setVideoHwAccelerationOptions( Fix됨
//                    rootEglBase.getEglBaseContext(), rootEglBase.getEglBaseContext()); Fix됨
//        }
    ...
    public void setAudioEnabled(final boolean enable) { // 오디오 켜기/끄기
        executor.execute(() -> {
            enableAudio = enable;
    ...
    public void setVideoEnabled(final boolean enable) { // 비디오 켜기/끄기
        executor.execute(() -> {
            renderVideo = enable;
    ...
    // 모두 콜백 메소드. 자동으로 실행되는 메소드들. 건드릴 필요 없음.
