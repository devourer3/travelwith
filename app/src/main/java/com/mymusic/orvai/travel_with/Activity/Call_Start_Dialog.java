...
public class Call_Start_Dialog extends AppCompatActivity {
    ...
    private String user_uuid, nickname, pic_url, fcm_token;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        user_uuid = intent.getStringExtra("selected_user_uuid");
        ...
        retrofit = new Retrofit.Builder().baseUrl(URLs.FIREBASE_MESSAGING).addConverterFactory(GsonConverterFactory.create()).build();
        r_service = retrofit.create(API.class);
        setContentView(R.layout.activity_call_dialog);
        ButterKnife.bind(this);
        Retrofit_Builder.get_Aws_Api_Service().request_located_user_info(user_uuid).enqueue(new Callback<Attraction_Near_User_by_located>() {
            @Override
            public void onResponse(Call<Attraction_Near_User_by_located> call, Response<Attraction_Near_User_by_located> response) {
                nickname = Objects.requireNonNull(response.body()).getUserNickname();
                pic_url = Objects.requireNonNull(response.body()).getUser_pic_url();
                fcm_token = Objects.requireNonNull(response.body()).getUser_fmc_token();
                Glide.with(mCtx).load(pic_url).into(userProfileImage);
                userNickname.setText(nickname);
            }

            @Override
            public void onFailure(Call<Attraction_Near_User_by_located> call, Throwable t) {
    ...
    private void connectToRoom(String roomId, boolean commandLineRun, boolean loopback,
        ...
        Call_Start_Dialog.commandLineRun = commandLineRun; // CLI 환경으로 실행하는지 여부
    ...
    public void onViewClicked() {
        Retrofit retrofit = new Retrofit.Builder().baseUrl(URLs.FIREBASE_MESSAGING).addConverterFactory(GsonConverterFactory.create()).build();
        API r_service = retrofit.create(API.class);
        User user = SharedPreferences_M.getInstance(mCtx).getUser();
        WebRTC_Calling_Request_API.FCM_Data_Model FCMData_model = new WebRTC_Calling_Request_API.FCM_Data_Model(user.getUser_nickname(), user.getUser_pic_url(), user.getUser_uuid(), "call");
        WebRTC_Calling_Request_API webRTC_calling_request_api = new WebRTC_Calling_Request_API();
        webRTC_calling_request_api.setFCMData_model(FCMData_model);
        webRTC_calling_request_api.setPriority("high");
        webRTC_calling_request_api.setToken(fcm_token);
        Call<ResponseBody> responseBodyCall = r_service.request_calling(webRTC_calling_request_api);
        responseBodyCall.enqueue(new Callback<ResponseBody>() {
            @Override
            public void onResponse(Call<ResponseBody> call, Response<ResponseBody> response) {
                connectToRoom(SharedPreferences_M.getInstance(mCtx).getUser().getUser_uuid(), false, false, false, 0);
            }
