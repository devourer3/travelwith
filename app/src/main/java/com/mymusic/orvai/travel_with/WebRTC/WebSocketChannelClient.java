 ...
 *
 * <p>All public methods should be called from a looper executor thread passed in a constructor, otherwise exception will be thrown.
...
public class WebSocketChannelClient {
  ...
  // 이 멤버 변수를 제거하지 마십시오. 이것이 제거되면 관찰자는 가비지 수집을하게되고 이로 인해 테스트가 중단됩니다.
  ...
  // WebSocket은 대기열을 발송합니다. WebSocket 클라이언트가 등록되어 있지 않고 register() 호출에서 소비되면 메시지가 대기열에 추가됩니다.
   ...
   * WebSocket에서 전달되는 메시지에 대한 콜백 인터페이스입니다. 모든 이벤트는 루퍼 실행 스레드에서 전달됩니다.
   */
  ...
  private void sendWSSMessage(final String method, final String message) {
    ...
    httpConnection.send(); // HttpURLConnection 으로 보냄
  }
