 ...
 * 두 개의 IP 주소 간 직접 통신을위한 Web SocketChannel 클라이언트 대체. TCP 연결을 사용하여 두 클라이언트 간의 신호를 처리합니다.
...
public class TCPChannelClient {
   ...
   * TCP Connection에서 전달되는 메시지에 대한 콜백 인터페이스. 모든 콜백은 루퍼 실행자 스레드에서 호출됩니다.
   */
   ...
   * TCPChannelClient를 초기화합니다. IP가 로컬 IP 주소 인 경우 해당 IP에서 수신 서버를 시작합니다. 그렇지 않은 경우 대신 IP에 연결합니다.
   *
