...
public class FirebaseRTCClient implements AppRTCClient, ValueEventListener {
    ...
    public void onDataChange(DataSnapshot dataSnapshot) {
        ...
        if (!dataSnapshot.hasChild(myId)) {
            database.child("/channels/"+myChannal).child(myId).child("connected").setValue(true);
            //Connected
            handler.post(() -> {
    ...
    // 여기서 인텐트로 받아온 내 UUID 값을 만들어 주면 되지. 그리고 /channels/ + 내 UUID 로 바꿔주는 거야.
    @Override
    public void connectToRoom(RoomConnectionParameters connectionParameters) { // ConnectActivity 가져온 매개변수를 CallActivity 에서 connectToRoom() 메소드를 매개변수를 담아 호출했을 때.
        if (connectionParameters.loopback) {
        }
        myChannal = connectionParameters.roomId; // 매개변수 중 룸 아이디
        myId = Build.SERIAL;
        database.child("/channels/"+myChannal).addValueEventListener(this);
    }
    ...
    public void sendOfferSdp(SessionDescription sdp) {
        ...
        database.child("/channels/"+myChannal).child(myId).child("sdp").setValue(sdp);
    }
    ...
    public void sendLocalIceCandidate(IceCandidate candidate) {
        ...
        database.child("/channels/"+myChannal).child(myId).child("icecandidate").child("" + candidate.hashCode()).setValue(candidate);
    }
    ...
    public void sendLocalIceCandidateRemovals(IceCandidate[] candidates) {
        ...
        for (IceCandidate candidate : candidates) {
            database.child("/channels/"+myChannal).child(myId).child("icecandidate").child("" + candidate.hashCode()).removeValue();
        }
    }
    ...
    public void disconnectFromRoom() {
        ...
        database.child("/channels/"+myChannal).child(myId).removeValue();
        //database.child("/channels/firebase").child("disconnect").setValue(true);
        database.child("/channels/"+myChannal).removeEventListener(this);
        sdpAdded.clear();
    }
